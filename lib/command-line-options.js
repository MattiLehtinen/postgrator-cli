...
import parse, {
    optionDefinitions as extraDefinitions,
...
const optionDefinitions = (defaults = {}) => [
    ...
    },
    {
        name: "database",
        description:
            "Database name. In the case of sqlite3, this is the filename, or it defaults to :memory:",
        alias: "d",
        type: String,
    ...
    },
    {
        name: "password",
        description:
            "Password. If parameter without value is given, password will be asked.",
        alias: "p",
        type: String,
    ...
    },
    {
        ...
        description:
            "A pattern matching files to run migration files from. Default: 'migrations/*'",
        alias: "m",
        type: String,
        lazyMultiple: true,
    ...
    },
    {
        ...
        description:
            "Validates checksum of existing SQL migration files already run prior to executing migrations.",
        type: Boolean,
        ...(defaults.validateChecksum && {
            defaultValue: defaults.validateChecksum,
        }),
    },
    {
        name: "ssl",
        description:
            "Enables ssl connections. When using the mysql driver it expects a string containing name of ssl profile.",
        alias: "s",
...
export default (argv, defaults) => {
    const { cmd, _unknown: unknown } = commandLineArgs(commandDefinitions, {
    ...
    if (args.ssl === null && defaults.ssl === undefined) {
        // If no value is handed, treat it as boolean
        args.ssl = true;
    }
        ...
        content: [
            {
                ...
                example:
                    "postgrator 23 --host 127.0.0.1 --database sampledb --username testuser --password testpassword",
            },
            {
