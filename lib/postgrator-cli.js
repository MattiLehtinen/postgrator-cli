...
import parse, { sections } from "./command-line-options.js";
...
function getAbsolutePath(fileOrDirectory) {
    return (
        Array.isArray(fileOrDirectory) ? fileOrDirectory : [fileOrDirectory]
    ).map((targetPath) => {
                  ...
                  .catch(
                      tap.catch(
                          (e) =>
                              ...
                              Promise.reject(
                                  new Error(
                                      `Config file not found: ${loadPath}`,
                                  ),
                              ),
                      ),
                  )
...
const getPostgratorOptions = (options) => ({
    ...
    ...("migration-pattern" in options
        ? { migrationPattern: getAbsolutePath(options["migration-pattern"]) }
        : {}),
    ...("schema-table" in options
        ? { schemaTable: options["schema-table"] }
        : {}),
    ...("validate-checksum" in options
        ? { validateChecksum: options["validate-checksum"] }
        : {}),
});
...
export async function run(argv) {
    const { help, version, config, "no-config": noConfig } = parse(argv); // first parse, without defaults
    ...
    const client = await getClient(
        postgratorOptions.driver,
        await getClientOptions(options),
    );
    ...
    try {
        postgrator = new Postgrator({
            ...
            ...(client.execSqlScript ? { execSqlScript: client.execSqlScript } : {}),
        });
    } catch (e) {
        printUsage();
    ...
    return command(postgrator, postgratorOptions)
        ...
        .catch((e) =>
            Promise.reject(e && typeof e === "string" ? new Error(e) : e),
        );
}
