...
const getDefaults = (config) => {
    ...
    const pResult = config
        ? explorer
            .load(config)
            .catch(
                (err) => (err.code === 'ENOENT' ? Promise.reject(new Error(`Config file not found: ${config}`)) : Promise.reject(err)),
            )
        : explorer.search();
    return pResult.then((result) => result?.config ?? {});
};
