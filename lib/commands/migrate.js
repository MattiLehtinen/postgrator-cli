...
export const optionDefinitions = (defaults = {}, defaultOption = false) => [
    {
        name: "to",
        description:
            "Version number of the file to migrate to or 'max'. Default: 'max'",
        type: String,
        typeLabel: "version",
...
function checkMigrations(migrations, migrationPattern) {
    if (!migrations || migrations.length === 0) {
        return Promise.reject(
            new Error(`No migration files found from "${migrationPattern}"`),
        );
    }
...
export default async (postgrator, { to, migrationPattern }) => {
    ...
    logMessage(
        `migrating ${toVersion >= databaseVersion ? "up" : "down"} to ${toVersion}`,
    );
